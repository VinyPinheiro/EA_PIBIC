...
AquecimentoAnimation::AquecimentoAnimation(const string& next)
    : Level("aquecimento", next)
{
    ...
    add_component(GEAR, 590, 390);
	add_component(CANO, 720, 430, Cano::CURVO1);
	add_component(CANO, 760, 510, Cano::CURVO7);
...
void
AquecimentoAnimation::add_component(Type type, double x, double y, Cano::Tipo p)
{
    switch (type) {
    case CANO:
        {
...
void
AquecimentoAnimation::draw_self()
{
    Environment *env = Environment::get_instance();
...
void
AquecimentoAnimation::update_self(unsigned long elapsed)
{
