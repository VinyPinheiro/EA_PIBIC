...
Biooleo_desoxigenacaoAnimation::Biooleo_desoxigenacaoAnimation(const string& next)
    : Level("biooleo_pressurizacao", next)
{
    ...
    add_component(GEAR, 110, 40);
	add_component(CANO, 240, 90, Cano::CURVO1);
	add_component(CANO, 280, 170, Cano::RETO3);
	add_component(CANO, 280, 250, Cano::CURVO7);
	add_component(CANO, 360, 290, Cano::RETO1);
...
void
Biooleo_desoxigenacaoAnimation::add_component(Type type, double x, double y, Cano::Tipo p)
{
    switch (type) {
    case CANO:
        {
...
void
Biooleo_desoxigenacaoAnimation::draw_self()
{
    Environment *env = Environment::get_instance();
...
void
Biooleo_desoxigenacaoAnimation::update_self(unsigned long elapsed)
{
