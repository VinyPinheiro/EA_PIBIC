...
Biooleo_destilacaoAnimation::Biooleo_destilacaoAnimation(const string& next)
    : Level("Biooleo_destilacao", next)
{
    ...
    add_component(GEAR, 990, 50);
	add_component(CANO, 1120, 120, Cano::RETO1);
...
void
Biooleo_destilacaoAnimation::add_component(Type type, double x, double y, Cano::Tipo p)
{
    switch (type) {
    case CANO:
        {
...
void
Biooleo_destilacaoAnimation::draw_self()
{
    Environment *env = Environment::get_instance();
...
void
Biooleo_destilacaoAnimation::update_self(unsigned long elapsed)
{
