...
Bioquerosene_transesterificacaoAnimation::Bioquerosene_transesterificacaoAnimation(const string& next)
    : Level("bioquerosene_transesterificacao", next)
{
    ...
    add_component(CANO, 0, 360, Cano::RETO1);
    add_component(GEAR, 70, 270);
	add_component(CANO, 200, 360, Cano::CURVO1);
	add_component(CANO, 240, 440, Cano::RETO3);
	add_component(CANO, 240, 520, Cano::CURVO7);
...
void
Bioquerosene_transesterificacaoAnimation::add_component(Type type, double x, double y, Cano::Tipo p)
{
    switch (type) {
    case CANO:
        {
...
void
Bioquerosene_transesterificacaoAnimation::draw_self()
{
    Environment *env = Environment::get_instance();
...
void
Bioquerosene_transesterificacaoAnimation::update_self(unsigned long elapsed)
{
